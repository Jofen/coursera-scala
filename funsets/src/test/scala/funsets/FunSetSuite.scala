...
class FunSetSuite extends FunSuite {
  ...
  test("filter function test"){
    new TestSets{
      ...
      val filtered = filter(s, x => (x == 1 || x == 2))
  ...
  test("forall function test"){
    new TestSets{
      val s = union(s1, s2)
      val ss = union(s2, s3)
      assert(forall(s, x => (x >=2)) == false, "Forall Fail")
      assert(forall(ss, x => (x >=2)) == true, "Forall Pass")
    }
  }
  ...
  test("exists function test"){
    new TestSets{
      val s = union(s1, s2)
      assert(exists(s, x => (x > 1)), "Exists Pass")
    }
  }
  ...
  test("map function test"){
    new TestSets {
      ...
      assert(forall(s, x => (x % 2) == 0), "Map Pass")
    }
  }
}
